	...
	module.exports = new Class( {
		...
		, s3AccelerateHostname: ".s3-accelerate.amazonaws.com"
    ...
    , listCommonPrefixes: function( prefix, delimiter, callback, offset ) {
      ...
      var isCommonPrefixesCommand = (delimiter != undefined);
			request( {
				  url:  		"https://" + this.credentials.bucket + ".s3.amazonaws.com/?prefix=" + path.substr( 1 ) + "&delimiter="+delimiter
				, method: 		"GET"
		...
		, getAccelerate: function(path, callback, url) {
			// Added by Garry Lachman (garry@lachman.co) https://github.com/garrylachman
			this.execute( "get", [path, callback, url, true]);
		}

		, _get: function( path, callback, url, accelerate){
			var hostname = ( ! accelerate ) ? this.s3Hostname : this.s3AccelerateHostname;
			request( {
				  url:  		url || ("https://" + this.credentials.bucket + hostname + path)
				, method: 		"GET"
				, aws: 			this.credentials
				, encoding: 	null
				, timeout: 		60000 // 60 secs
		...
		, initialize: function(){
			[ "get", "put", "delete", "list", "head", "listCommonPrefixes", "getAccelerate" ].forEach( function( action ){
